...
class Beam:
    def __init__(self, 
        ...
        self.init_edges()
        if self.use_regions:
            self.init_random_regions(min_regions=3, max_regions=5, seed=seed)
        else:
    ...
    def init_pos(self):
        for i, j in ti.ndrange(self.N_row, self.N_column):
            ...
            # Calculate spacing to fit grid in window while maintaining reasonable size
            spacing = 0.05  # Fixed spacing between nodes
            
            # Calculate offsets to center the grid
            x_offset = 0.5 - (self.N_column - 1) * spacing / 2
            y_offset = 0.6 - (self.N_row - 1) * spacing / 2
            
            # Position nodes with centered offsets
            self.pos[idx] = ti.Vector([x_offset + j * spacing, y_offset + i * spacing])
    ...
    def compute_force(self):
        ...
        ground_y = 0.1
        ground_k = 1.0e4  # Ground stiffness
        # ground_d = 10.0  # Ground damping is fixed
        ground_d = self.kd
        
        for i in range(self.NV):
            if self.pos[i][1] < ground_y:
                # Spring force from ground
                penetration = ground_y - self.pos[i][1]
                ground_force = ti.Vector([0.0, ground_k * penetration])
                # Damping force
                if self.vel[i][1] < 0:
                    ground_force += ti.Vector([0.0, ground_d * (-self.vel[i][1])])
                self.force[i] += ground_force
    ...
    def compute_Jacobians(self):
        for i in self.spring:
            ...
            # Use average damping of connected nodes
            kd_avg = (self.damping_matrix[idx1] + self.damping_matrix[idx2]) * 0.5
            self.Jv[i] = kd_avg * I
    ...
    def display_kernel(self, vertices: ti.template(), colors: ti.template()):
        ...
        for i, j in ti.ndrange(self.N_row-1, self.N_column-1):
            ...
            if self.use_regions:
                ...
                blue = 0.2 + 0.6 * (self.kd - self.kd_range[0]) / (self.kd_range[1] - self.kd_range[0])  
...
def save_video(beam, h, video_name="simulation", data_dir=None):
    ...
    for frame in range(total_frames):
        ...
        # Collect frames
        if frame % 20 == 0:  # Save every 10th frame
            # Render frame
...
def simulate(beam, save_to_video=False, video_name="simulation", data_collection=False, data_dir=None):
    h = 0.005  # time step, smaller time step for better accuracy, maybe more "active"
    if data_collection:
        """data_collection version"""
        save_video(beam, h, video_name=video_name, data_dir=data_dir)
...
def run_simulation(param_idx, param, N_ROW, N_COLUMN, MASS_RANGE, STIFFNESS_RANGE, DAMPING_RANGE, dataset_dir, DATA_COLLECTION):
    ...
    try:
        ...
        if param == 0:
            ...
            # print(f"param: {param}")
            # Create beam instance with current parameters
            beam = Beam(N_row=N_ROW, 
                        N_column=N_COLUMN, 
                        mass_range=MASS_RANGE,
                        stiffness_range=STIFFNESS_RANGE,
...
def main():
    MASS_RANGE = [1, 0.1]
    STIFFNESS_RANGE = [20000.0, 200000.0]
    DAMPING_RANGE = [1, 50]
    N_ROW = 11
    N_COLUMN = 11
    DATA_COLLECTION = False
    BLOB_TYPE = "region" # "region" or "uni"
    ...
    if DATA_COLLECTION:
        if BLOB_TYPE == "region":
            dataset_dir = os.path.join(os.path.dirname(__file__), 'dataset_blob_region')
        elif BLOB_TYPE == "uni":
            dataset_dir = os.path.join(os.path.dirname(__file__), 'dataset_blob_uni')
        else:
    ...
    if BLOB_TYPE == "uni":
        N_sample = 22 # N_sample ** 3 dataset
