...
if __name__ == "__main__":
    epoch_nums = [1, 243]
    len_epoch = len(epoch_nums)
    for epoch_num in epoch_nums:
        img_dir = os.path.join(ROOT, file_name,"output_b1_tiny","images")
        if not os.path.exists(img_dir):
            print(f"File {img_dir} does not exist. Please check the path.")
            exit(1)
        ...
        pred_files = [
            f for f in all_files
            if f.startswith("pred") and f"epoch{epoch_num}_" in f
        ]
        gt_img_paths = [os.path.join(img_dir, f) for f in gt_files]
        pred_img_paths = [os.path.join(img_dir, f) for f in pred_files]

        num_imgs = len(gt_img_paths)
        for i in range(num_imgs):
            output_dir_tmp = os.path.join(output_dir, f"epoch_{epoch_num}", f"batch{i}")
            img_root_gt = gt_img_paths[i]
            img_init = Image.open(img_root_gt)
            ...
            simulator = MPMBeamSimulator(n=64, num_particles=4900, dt=1e-4, frames=frames, steps_per_frame=steps_per_frame, output_dir=output_dir_tmp, create_video = True)
