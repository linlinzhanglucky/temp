...
class MPMBeamSimulator:
    ...
    def build_regions(self, num_regions):
        ...
        for k in range(num_regions):
            E = np.random.uniform(12.0e4, 26.0e4)
            nu = np.random.uniform(0.01, 0.45)
            rho = np.random.uniform(1.0, 4.0)
            self.region_E[k] = E
            self.region_nu[k] = nu
    ...
    def run(
        ...
        gui = ti.GUI("MPM Beam", res=self.gui_res, background_color=0xFFFFFF, show_gui=False)
        ...
        for idx in range(10000):
            ...
            self.build_regions(np.random.randint(2,5))
            self.initialize()
            ...
            for frame in range(self.frames):
                ...
                if frame == 0:
                    ...
                    gui.circles(self.x.to_numpy(), radius=3.0, color = 0x000000)
...
if __name__ == "__main__":
    simulator = MPMBeamSimulator(n=64, num_particles=4900, dt=1e-4, frames=25, steps_per_frame=300, output_dir="mpm_output_0528_blob", create_video = True)
    Es = [12.0e4, 26.0e4]
    Nus = [0.01, 0.45]
    Rhos = [1.0, 4.0]
