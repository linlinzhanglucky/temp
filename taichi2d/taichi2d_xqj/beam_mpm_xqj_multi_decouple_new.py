...
dt = 1e-4  # Time step
num_samples = 20000#10000  # Number of samples to generate
num_channels = 2  # RGB channels for color

if num_channels == 2:
    Nu_mean = 0.5 * (Nu_max + Nu_min)
...
class MPMBeamSimulator:
    def __init__(
        ...
        def initialize(
            ...
            for i in range(self.num_particles):
                ...
                if i == 0:
                    base_mass = mass
                    base_rho = base_mass / ((self.dx * 0.5) ** 2)
                self.E_particle[i] = E
    ...
    def build_regions(self, num_regions):
        ...
        target_point = np.array([xmin, ymax])
        ...
        for k in range(num_regions):
            if decouple:
                r = self.norm_color(self.region_E[k]/(self.region_mass[k] / (self.dx * 0.5) ** 2), E_to_Rho_min, E_to_Rho_max)
            else:
                r = self.norm_color(E, E_min, E_max)
            # r = self.norm_color(E, 12.0e4, 26.0e4)
            g = self.norm_color((self.region_mass[k] / (self.dx * 0.5) ** 2) / base_rho, green_min, green_max)
