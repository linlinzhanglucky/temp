...
@ti.data_oriented
class voxelSobo:
    def __init__(self, np_vertices, np_edges, np_stiffness, np_faces, np_face_ver, big_size = 1, real_size = 1):
        self.dt = 0.001      
        self.mass = 1.0   
        self.k_spring = 100000.0 
        self.damping = 2.0 
        ...
        self.np_stiffness = np_stiffness
        ...
        self.stiffness = ti.field(dtype=ti.f32, shape=self.num_edg)
        ...
        self.stiffness.from_numpy(np_stiffness)
    ...
    def substep(self):
        ...
        for e in range(self.num_edg):
            ...
            F = self.stiffness[e] * elongation * dirAB
    ...
    def run(self):
        ...
        for j in range(int(10/self.dt)):
            if window.running:
                ...
                # 自定义颜色（这里用 normalized position 映射为 RGB）
                #colors = (pos - pos.min(0)) / (pos.max(0) - pos.min(0) + 1e-8)  # normalize 到 [0,1]
                colors = np.tile(np.array([[0.2, 0.6, 1.0]]), (pos.shape[0], 1))
                pcd.colors = o3d.utility.Vector3dVector(colors)
                print(np.asarray(pcd.colors))
                #save pcd to local folder
                o3d.io.write_point_cloud("./pcd/pcd_" + str(i) + ".ply", pcd)
...
if __name__ == "__main__":
    ...
    np_stiffess = blob.assign_stiffness(points = np_vertices, num_samples = 5, new_edges = np_edges)
    voxelSobo_1 = voxelSobo(np_vertices = np_vertices, np_edges = np_edges, np_stiffness = np_stiffess,
                             np_faces = np_faces, np_face_ver = np_face_ver, big_size=r, real_size = real_size)
    ...
    # #BALL
    # radii = [8, 7, 6, 5,3, 0.4, 0.3, 0.2, 0.1]
